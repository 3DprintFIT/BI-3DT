...
{{:classification:bi-3dt.png|}} {{:classification:trubka.png?250}} {{:classification:trubka2.png?250}}
...
<code lang="cpp">module splitter(pipes=[...], wall_thick=2.5) { ... }</code>
...
{{:classification:profiles.png|Profily}}
...
{{:classification:top-view.png|Pohled z vrchu}}
...
{{:classification:front-view.png|Pohled z boku}}
...
<note tip>Pozor, pokud bude vzdálenost od středu příliš krátká a konce trubek by se překrývali, smíte (musíte) je patřičně prodloužit, nikoli však více, než je potřeba. Tuto skutečnost řádně okomentujete. Není nutné tento stav ošetřovat v parametrickém modelu!</note>

<note important>Vaše řešení v OpenSCADu musí podporovat všechny profily trubek, ne jen ty, které máte zadány.</note>
...
<note tip>Nejde vám v Meshmixeru dobře alignovat? Zkuste program [[https://www.lulzbot.com/cura|Cura]]. Obsahuje funkci //Lay flat//.</note>
...
<note important>Pro ovládání tiskárny při odevzdávání potřebujete vlastní počítač se schopností připojit se na WiFi nebo síťovým kabelem do lokální sítě. Také potřebuje znát (umět dohledat) svou MAC adresu.</note>
