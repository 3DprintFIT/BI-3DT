...
GCode existoval dříve, než 3D tiskárny (první implementace se objevila v 50. letech) a byl používán pro https://en.wikipedia.org/wiki/Numerical_control[CNC stroje] (3D tiskárna je ovšem v podstatě také CNC stroj). Pro účely 3D tisku jsou do GCodu přidány https://reprap.org/wiki/G-code#M-commands[specifické kódy] (začínající na `M`), které definují ovládání funkcí, které jsou určeny pro 3D tisk (teplota trysky, průtok plastu).

Každý řádek představuje instrukci. Ta je popsána kódem, který je uveden jako první (např. `G1`) a za ním následují parametry (některé instrukce parametry nemají) oddělené mezerou (např. `E4.00000 F3000.00000`).
...
Slicerů existuje řada, některé jsou lepší, jiné horší, některé mají dobré UI ale neodvádějí tak dobrou práci, jiné jsou zase výbornými slicery ale mají příšerné UI. Najdou se i ty, které disponují obojím. Slicer může být samostatný software, nebo některé softwary mají integrovaný slicer v sobě.

V předmětu BI-3DT budeme k výuce používat program *Slic3r*. Řadí se mezi nejpoužívanější, je open-source a kvalita výstupu je dobrá.
...
http://slic3r.org/download[Ke stažení na stránce slic3r.org].
...
Na cvičení budeme používat poslední verzi Slic3ru (>=1.3.0). V této verzi lze jednotlivé panely otevřít v nabídce `Settings`.

.Výchozí okno programu Slic3r (verze 1.3)
image::../images/slicing/slic3r.png[Okno Slic3ru, width=720, link=../images/slicing/slic3r.png]

[[config-file]]
==== Konfigurační soubor pro tiskárny

.Položka "Load Config Bundle"
...
Tento soubor načtěte pomocí `File > Load Config Bundle`. Po načtení config bundlu je často nutné program restartovat. Úspěšné načtení config bundle poznáte podle nových položek vpravo nahoře (ve filamentu se objeví na výběr "ABS ESUN 1.75mm" a v tiskárnách "RebeliX").
...
V minulých cvičeních jste se dozvěděli, že meshí popisujeme pouze plášť objektu, nikoliv jeho vnitřek. Fyzicky na tiskárně však nelze vytisknout pouze tento plášť. Slicer se postará o vytvoření správné výplně objektu. Tato výplň se označuje slovem _"infill"_.

Předměty se běžně netisknou duté (vršek objektu by se neměl na co tisknout), ani plné (plýtvání filamentem a vzniká v předmětu pnutí, které narušuje integritu výtisku). Vyšší hustota infillu dělá předmět odolnější, těžší, ale také dražší a déle se tiskne. Nízká hustota naopak může zapříčinit nevzhledný výtisk (plášť objektu se může na některých místech propadat). V praxi se infill nenastavuje na nižší hodnotu než 10%.
...
Často lze tyto problémy řešit otočením modelu, nebo jeho rozdělením na více částí a následným ručním slepením po dokončení tisku. Někdy takové úpravy však nejsou možné a tak může slicer přidat k našemu modelu podpůrné struktury, které se po dokončení tisku odlomí, odřežou či jinak odstraní.

[[structs]]
=== Rafts, skirts, brims

Určité typy výtisků mají problém s přilnavostí k povrchu tiskové plochy. Uprostřed tisku se pak mohou odlepit a v tu chvíli se tisk stává neúspěšným.
...
Na panelu plater pracujete s objekty, které chcete tisknout. Tlačítkem `Add` můžete přidat STL soubor. V 3D náhledu můžete objekty rozložit po tiskové ploše. Dalšími tlačítky ve vrchním panelu můžete vybraný objekt manipulovat (otáčet tlačítky `Rotate`, či měnit jeho velikost tlačítkem `Scale`).
...
Dole lze vybírat z několik pohledů, pro nás jsou především důležité pohledy "3D" a "Preview". V 3D pohledu lze pracovat s rozpoložením objektů, pohled "Preview" vám ukáže, jak bude výsledný tisk vypadat, jaké struktury se vytisknou a lze vidět i náznak jednotlivých vrstev.

=== Panel "Print Settings"

.Panel "Print Settings"
image::../images/slicing/print-settings.png[Print Settings, width=320, link=../images/slicing/print-settings.png, float=right]

Panelu "Print Settings" věnujte největší pozornost. Zatímco většinu důležitých hodnot v ostatních panelech za vás nastavil <<Konfigurační soubor pro tiskárny, konfigurační soubor>>, který jste si stáhli výše, nastavení tisku bude vaši hlavní úlohou při práci na cvičeních.
...
==== Stránka "Layers and perimeters"

Layer height:: Ovlivňuje výšku vrstvy -- přímo ovlivňuje kvalitu a dobu trvání tisku. Rozsah smysluplných hodnot ovlivňuje průměr trysky, která je nainstalovaná na tiskárně.
...
Solid layers:: Počet vrstev s plnou výplní na spodku a vršku. Čím více vrstev zvolíme, tím pevnější bude vrchní a spodní povrch a tím lépe se podaří zakrýt nastavený infill. Vysoká čísla však zvýší dobu tisku.
...
Solid infill every:: Zesílí odolnost předmětu vytisknutím plné výplně (100%) každých n vrstev.

Fill angle:: Úhel vzorku výplně.

Solid infill threshold area:: V malých částech předmětu (např. nějaké tenké trubičky či spojovník) slicer vyplňuje plnou výplní (100%). Tato hodnota určuje práh při které se část předmětu považuje jako malá.
...
Panel "Filament Settings", jak název napovídá, skrývá nastavení vztahující se k filamentu. Ze jsou nejdůležitějšími položkami "Diameter" určující průměr struny a nastavení teplot, které se liší podle materiálu (jak se dozvíte v prvním cvičení věnující se tisku). Na stránce "Cooling" pak lze definovat chování ventilátoru k dodatečnému chlazení výtisku (některé materiály jej potřebují).

Panel "Printer Settings" slouží k nastavení parametrů tiskárny (velikost a tvar tiskové plochy, počet extruderů atd.). Zatímco položky na panelech "Print Settings" a "Filament Settings" měníte prakticky při každém tisku, položky na tomto panelu zůstávají povětšinou netknuté, dokud nezměníte konfiguraci tiskárny.
