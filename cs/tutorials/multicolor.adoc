...
== Možnosti vícebarevného tisku

Existuje několik možností, jak vícebarevný tisk provést.

== Sdílené informace (pro všechny možnosti)

IMPORTANT: Pro dnešní cvičení si stáhněte oba obecné konfigurační soubory pro OrcaSlicer (pro Voron i Hybrix). Odkazy najdete v xref:printing#config-file[sekci o konfiguraci v návodu k tisku].
...
...
----
<1> komentář vygenerovaný slicerem, ne v každém Gcodu musí být, může se lišit mezi modely tiskáren
<2> tento řádek je zásadní -- *za ním* soubor rozdělíme
<3> komentář vygenerovaný slicerem, ne v každém Gcodu musí být, může se lišit mezi modely tiskáren
...
== Vložení GCode příkazu pro výměnu filamentu

Pro výměnu filamentu můžete využít Gcode příkaz
...
Po výměně plastu stačí stisknout tlačítko (buďto na displeji, nebo v webovém rozhraní), tisková hlava se vrátí zpět a tisk pokračuje.
Místo rozdělení souboru na dva pak stačí do Gcodu přidat jeden řádek navíc, většina moderních slicerů na to má zabudovanou funkci.

1. Připravte si model k tisku a nastavení
2. V náhledu si najděte vrstvu, kde chcete vyměnit filament
3. Klikněte pravým tlačítkem na ikonu + a vyberte `Add Custom GCode`
4. __Pokud používáte tiskárnu Hybrix, vložte do políčka `Custom GCode` následující příkaz:__ `M226`
5. __Pokud používáte tiskárnu Voron, vložte do políčka `Custom GCode` následující příkaz:__ `M600`
6. Vyexportujte GCode a model vytiskněte.


== Úkol

Vytiskněte libovolné množství (nejméně však jeden) dvoubarevného
link:../stls/multicolor/3dprintlab-logo.stl[loga laboratoře]. Nastavení
tisku zvolte dle potřeby, vycházejte však z tiskového profilu _0.2 NORMAL_.
Domluvte se s kolegy v učebně na výměně barvy filamentu,
případně požádejte cvičící o náhradní barvu.

IMPORTANT: Vysvětlete cvičícímu, jak jste postupovali.
