...
There are several options for performing multi-color printing.

== Shared Information (for all options)

IMPORTANT: For today's exercise, download both general configuration files for OrcaSlicer (for Voron and Hybrix). Links can be found in the xref:printing#config-file[configuration section of the printing tutorial].
...
...
----
<1> comment generated by the slicer, may not be in every Gcode, may differ between printer models
<2> this line is crucial -- we split the file *after it*
<3> comment generated by the slicer, may not be in every Gcode, may differ between printer models
...
== Inserting a GCode Command for Filament Change

For filament change you can use the Gcode command
...
After changing the plastic, just press the button (either on the display or in the web interface), the print head returns, and printing continues.
Instead of splitting the file into two, it's enough to add one extra line to the Gcode; most modern slicers have a built-in function for this.

1. Prepare your model for printing and settings
2. In the preview, find the layer where you want to change the filament
3. Right-click on the + icon and select `Add Custom GCode`
4. __If you are using a Hybrix printer, enter the following command in the `Custom GCode` field:__ `M226`
5. __If you are using a Voron printer, enter the following command in the `Custom GCode` field:__ `M600`
6. Export the GCode and print the model.
...
printing settings as needed, but base them on the _0.2 NORMAL_ print profile.
Arrange filament color exchange with classmates in the classroom,
or ask instructors for an alternative color.

IMPORTANT: Explain to the instructor how you proceeded.
