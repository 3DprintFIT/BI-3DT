...
Basic *primitives* (sphere, cone/cylinder, cuboid…) are composed into a *CSG tree*
using edges of *transformations* (translation, scaling, rotation…)
...
There's a fast (not always accurate) preview using OpenGL (_Design → Display_, kbd:[F5])
and a full render to 3D model for export (unfortunately calculated on CPU,
_Design → Render_, kbd:[F6]).
...
You can choose from a rich selection in _File → Examples_.
...
`multmatrix([[...]])`::
  transformation matrix of size 4×4, low-level
...
The result is twice printed `ECHO: 4242` 🤯
...
.Minkowski sum ©  https://en.wikipedia.org/wiki/File:Сумма_Минковского.svg[Tosha] (Public Domain)
...
(e.g., `translate()` takes a vector of 2 values, `rotate()` takes only one angle…).
...
for the model to be Simple – https://en.wikipedia.org/wiki/2-manifold[2-Manifold].
